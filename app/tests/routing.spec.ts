...
function fakeCodexEnvironment(_project: string, connected: boolean, behavior: "success" | "invalid-jsonl" | "missing-claims" | "slow" = "success"): { env: NodeJS.ProcessEnv; marker: string; prompt: string } {
...
// Three app-side gates have to thread `details` for this to pass — the route
// disclosure, the run-time disclosure gate, and the authorization handed to
// the adapter — and a fourth (runSerialTask's own options) has to carry it
// into the contract, or the worker is handed a request the owner never
// confirmed. Miss any one and this test fails: the earlier three refuse the
// run outright with REAL_MODEL_CALL_NOT_AUTHORIZED, the last strips the
// owner's numbers out of the prompt the fake codex writes down.
