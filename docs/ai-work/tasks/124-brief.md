# Task 124 brief — Pin Kimi K3, clarify the picker, open the add-a-model path

Implements the owner decisions recorded in
`docs/ai-work/HANDOFF-model-picker.md` (2026-07-29).

Renumbered from 123 after a double-claim: this lane committed its brief first
(046186a), the Soft festival palette lane claimed the same number later
(8a52831) and landed its Task 123 first (0535919). With both lanes mid-landing
in one shared tree, this lane took 124 — the smallest move that removes the
race. Original claim commit 046186a stands as history.
...
- Files in scope: `app/src/renderer/bodies.ts`,
  ...
  `app/tests-unit/bodies.test.ts`), plus task records.
...
- The parallel lane's work — its Task 123 palette port to `tokens.css`
  (landed 0535919) and the untracked `design/` scratch area — is never
  touched, staged, or committed. Its uncommitted tokens.css sat in the tree
  while this lane's checks ran (mixed-tree caveat in the report).
