...
- No IPC, core, CLI, contract, or dependency changes. No change to what
  ...
  this task's).
- The parallel lane's in-flight Task 125 work (uncommitted `tokens.css`,
  untracked `design/`) is never touched, staged, or committed; its presence
  in the tree during checks gets the usual mixed-tree disclosure.
