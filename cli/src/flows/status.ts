...
import { banner, stack } from "../ui.js";
...
export function statusFlow(root: string): void {
  ...
  if (status.legacyState) console.log(`\n${pc.yellow("Legacy runtime state is preserved. New task mutation is blocked until a safe migration exists.")}`);
  ...
  if (status.unfinished) console.log(`\n${pc.yellow("○")} Task ${String(status.unfinished.taskNumber).padStart(3, "0")} has retained records and no closing log row. It remains visible but does not block a new task.`);
}
